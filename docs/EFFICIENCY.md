...
|---|---|---|---|---|
| 1 | Scope the task gate to the workspaces in `task.areas` (`npm run lint -w <ws>`, `npx vitest run <ws>`); full gate stays at merge/verify | `gateScopeForAreas`, `gateCommands`, `verifyRepo({ areas })`, `workerLimits.scopedTaskGate` | 60–80 % of gate wall clock per run (1 workspace instead of 5) and 40–60 % of the gate output in context (no unrelated eslint/tsc noise). Per task: −10–20 % tokens, −30–50 % minutes | A task touching `apps/api` can break `apps/app`'s types only through `packages/*`, which keeps the full gate; a break elsewhere is caught by the merge gate, one step later, where the repair session is more expensive. `packages/*` tasks are not scoped. |
...
| 4 | `maxTurns` per task by size: S 60 / M 100 / L 150 (was 200 flat), repair session 60 as the safety valve; the cap shows up in the `task_failed` reason (`turn cap 60 for size S reached`) | `workerLimits.maxTurnsBySize`, `maxTurnsForSpec`, `runTask` | Bounds the worst case: an S task can no longer spend 200 turns (≈ 2M budget-tokens). A capped session is committed and gated; the repair session gets the gate output and the cap note, so the last mile usually still lands | A legitimately long task is cut off and the repair session has to finish it blind. The size class is the price estimate's, so the customer's budget and the cap agree. |
| 5 | Prompt caching: the SDK already caches system prompt + tools + prefix; the harness now guarantees no `DISABLE_PROMPT_CACHING*` switch reaches a session (`sessionEnv`) and keeps the system prompt byte-identical between a task's worker and repair session (same `scope`, computed once) | `sessionEnv`, `runTask` | No change vs today when caching was on (the 10 % weight assumes it); a 10× input-cost protection if it ever is not. Repair session starts from a warm cache instead of re-writing the prefix (~4k tokens × 1.25) | None. |
...
- gate runs per task (grep the Bash tool calls for `vitest`/`lint`; target ≤ 2 by the worker + 1 by
  the harness),
- `cache_read_input_tokens` > 0 from the second turn of every session (caching on),
- wall clock per task.
