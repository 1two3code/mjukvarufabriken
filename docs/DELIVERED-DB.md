...
- ~~**Teardown of provisioned databases**~~ — done in wave 14 (hosting window, strategy F4):
  `previewDbService.teardown(jobId)` drops the database (`WITH (FORCE)`) and its role inside
