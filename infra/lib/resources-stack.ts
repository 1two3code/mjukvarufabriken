...
export class ResourcesStack extends Stack {
	...
	constructor(scope: Construct, id: string, props: ResourcesStackProps) {
		...
		// Manager, the artifacts bucket and the api bypass the proxy (NO_PROXY, exact hosts — a
		// wildcard `.amazonaws.com` would let any AWS-hosted endpoint skip the allowlist);
		// everything else, including every other AWS service, must pass the allowlist.
		...
		// MARK: Job task role — reviewed M9, narrowed by M3 hardening. The container runs
		// customer-driven code, so it gets exactly what apps/job needs today:
		...
		// (docs/M3-REVIEW.md #18). Never: Stripe keys, the auth signing key, the GitHub token (M5
		// delivery mints a short-lived per-job token instead), ecs:* or logs:* (the execution role
		// writes logs). Secrets reach the container as ARNs and are fetched at start-up.
		//
