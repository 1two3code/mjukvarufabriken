...
| Stack | Contents |
|---|---|
| `resources-<env>` | VPC (2 AZs, public + private-with-NAT + isolated subnets, **1 NAT gateway**), RDS **Postgres 17** (`mf` database, credentials generated into Secrets Manager, isolated subnets), `artifacts` bucket (versioned, private, lifecycle: abort multipart after 7 d, expire old versions 14 d dev / 90 d live), four Secrets Manager placeholders (below), ECS cluster `mf-jobs-<env>` + Fargate task definition `mf-job-<env>` for build jobs (M3; placeholder `node:24-alpine` image, 2 vCPU / 4 GB, logs 14 d). Exports `vpc-id`, `rds-endpoint`, `rds-secret-arn`, `s3-artifacts`, `ecs-jobs-cluster-arn`, `ecs-job-task-definition-arn`, `ecs-job-security-group-id`. |
| `mf-<env>` | **Site + Portal**: one private S3 bucket + CloudFront each (OAC, SPA fallback, security headers). **API**: ECS Fargate behind an ALB from `apps/api/Dockerfile` in the private (NAT) subnets, health check `/health`, security-group ingress to Postgres :5432, env vars `DATABASE_SECRET_ARN`, `ARTIFACTS_BUCKET`, `JOBS_CLUSTER_ARN`, `JOB_TASK_DEFINITION_ARN`, `JOB_SUBNET_IDS`, `JOB_SECURITY_GROUP_ID`, `STRIPE_*_SECRET_ARN`. Task role: read db + Stripe secrets, read/write artifacts, `ecs:RunTask/DescribeTasks/StopTask` scoped to the jobs cluster + `iam:PassRole` for the job roles. Optional custom domains + Route 53 records. |
