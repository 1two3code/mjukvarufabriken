...
`github-token` is read by build jobs (M5 delivery: create `mjukvaruhuset/<repo>`, push, invite the customer) — a classic token with `repo` + `admin:org`, or a fine-grained one with contents/administration write on the org. App Runner previews additionally need a GitHub connection ARN in `config.ts` (`appRunner.connectionArn`, TODO-EXTERNAL); the job passes the empty `mf-apprunner-instance-<env>` role (`apprunner-instance-role-arn` output) to every service it creates.
