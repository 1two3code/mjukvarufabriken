...
1. **Region lock** — deny every action whose `aws:RequestedRegion` is not `eu-north-1` (our
   ...
   that only live there). Only genuinely global/partition-wide services (IAM, Organizations, STS,
   CloudFront, Route 53, Budgets, …) are exempted via `NotAction` so the lock doesn't brick them.
   ACM and KMS are regional, and ACM-for-CloudFront lives in `us-east-1` — already an allowed
   region — so neither is exempted.
...
per-customer or per-env pipeline. `AWS::Organizations` resources deploy only through the
`us-east-1` endpoint, so the stack is pinned there — pass `--region us-east-1` (the stack env
sets it too, but bootstrap and the CLI still need it).

```sh
npm i --prefix infra/org
cd infra/org
npx cdk bootstrap --region us-east-1              # once per account, if not already bootstrapped
npx cdk deploy --region us-east-1 -c rootId=r-hh2k  # creates the Customers OU + attaches the SCP
```
