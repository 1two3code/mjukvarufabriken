...
/** The site's port of the portal's spec preview (same copy) over the anonymous quote */
export function QuotePreview({ quote }: QuotePreviewProps) {
	const { t, i18n } = useTranslation()
	const { spec, complete } = quote
	const statusClass =
		quote.status === 'frozen' ? styles.frozen : complete ? styles.complete : styles.incomplete
	...
	return (
		<section className={styles.preview}>
			...
			{quote.sizeClass && quote.priceSek !== undefined && (
				<div className={styles.price}>
					<span className={styles.sizeClass}>
						{t('spec.preview.sizeClass', { size: quote.sizeClass })}
					</span>
					<span className={styles.priceValue}>
						{t('spec.preview.price', { price: quote.priceSek.toLocaleString(i18n.language) })}
					</span>
				</div>
			)}
