...
describe('Delivery timeline', () => {
	...
	it('Judges each step by its LAST event and carries reason and url', () => {
		const timeline = deliveryTimeline([
			...
			event(6, { step: 'acceptance', ok: false, reason: 'blank page' }),
			event(7, { step: 'bundle', ok: true, url: 's3://bundle' }),
		...
		expect(timeline).toEqual([
			...
			{ step: 'bundle', state: 'ok', reason: undefined, url: 's3://bundle' },
		])
