...
describe('Anthropic forward proxy', () => {
	...
	// Regression: dogfood run 1 (2026-09-01) failed every build at `planning failed: terminated`.
	// `fetch` (undici) transparently DECOMPRESSES an upstream gzip body, but leaves
	// `content-encoding: gzip` on the response headers. Forwarding that header alongside the
	// already-decoded bytes hands the caller plain JSON labelled as compressed — it fails to
	// inflate and the request dies. The stub therefore has to mimic undici: decoded body, header
	// still present. Without the fix this test fails on the forwarded header.
	it('Does not forward content-encoding — the body it relays is already decoded', async () => {
		// Arrange
		...
		const proxy = await startAnthropicForwardProxy({
		...
		// Assert — the caller must be able to read the body, and never see a stale encoding claim
