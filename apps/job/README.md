...
How the switch works (documented choice): the container starts as root only for `setpriv` to drop to `node` with `CAP_SETUID`/`CAP_SETGID` as **ambient** capabilities and a bounding set cut to those two (`apps/job/Dockerfile` `CMD`). The harness (`@mf/harness` `launch` in `exec.ts`) spawns every child through `setpriv --inh-caps=-all --ambient-caps=-all --no-new-privs`, adding `--reuid=worker --regid=worker --init-groups` for worker sessions and repo scripts — so the two capabilities exist in the node process alone; a worker runs with an empty capability set and `no_new_privs`, which also blocks any setuid binary. The Agent SDK session is spawned through its `spawnClaudeCodeProcess` hook with the same wrapper, with `HOME=/home/worker` and its own `CLAUDE_CONFIG_DIR`. Files are shared through the `work` group instead of chown ping-pong: `/work` is `node:work` with the setgid bit, the job writes with `umask 002`, git runs with `core.sharedRepository=group` + `safe.directory=*` (via `GIT_CONFIG_*`), and `shareWithWorker` puts the group write bit on every tree before a worker first runs in it (the template copy and `cp -al` produce stricter modes). `WORKER_UID` unset (or equal to the job's uid) keeps the single-user behaviour, so `npm run job:dev` and the tests need nothing.

Manual check (the uid switch cannot be exercised in unit tests, `docker compose --profile job build` first). It runs the exact privilege chain of the image — root → `node` with ambient caps → worker — and prints what a worker session can and cannot do:
...
run(['--reuid=1001', '--regid=1001', '--init-groups', '--inh-caps=-all', '--ambient-caps=-all', '--no-new-privs', '--', 'sh', '-c',
