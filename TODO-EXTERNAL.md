...
| App Runner GitHub connection: App Runner console → GitHub connections → create for the `mjukvaruhuset` org, complete the handshake, put the ARN in `infra/lib/config.ts` `appRunner.connectionArn` per env (M5 deploys the customer api from the pushed repo; without it delivery reports `deployUrl: null` + a notify) | you | minutes | M5 preview URL |
